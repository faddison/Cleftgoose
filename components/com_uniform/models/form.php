...
class JSNUniformModelForm extends JModelItem
{
	...
	public function save($post)
	{
		...
		if (!empty($dataForms->form_captcha) && $dataForms->form_captcha == 1)
		{
					...
					if (isset($fieldSettings->options->required) && (int) $fieldSettings->options->required == 1)
					{
						switch ($colum->field_type)
						{
							...
							case "email":
								...
								break;
							case "number":
								if (empty($post['number'][$fieldName]['value']))
								{
									$validationForm['number'][$fieldName] = JText::_('JSN_UNIFORM_CONFIRM_FIELD_CANNOT_EMPTY');
								}
								else
								{
									...
									else if (!preg_match($regex, $postNumberDecimal))
									{
										$validationForm[$fieldName] = JText::_('JSN_UNIFORM_CONFIRM_FIELD_INVALID');
									}
								}
