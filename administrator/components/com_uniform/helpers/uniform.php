...
class JSNUniformHelper
{
	...
	public static function addAssets()
	{
		...
		// We need to manually load Bootstrap in Joomla 2.5
	...
	public static function addSubmenu($vName = 'forms')
	{
		...
		{
			JSubMenuHelper::addEntry(JText::_('JSN_UNIFORM_SUBMENU_FORMS'), 'index.php?option=com_uniform&view=forms', $vName == 'forms');
			JSubMenuHelper::addEntry(JText::_('JSN_UNIFORM_SUBMENU_SUBMISSION'), 'index.php?option=com_uniform&view=submissions', $vName == 'submissions');
			JSubMenuHelper::addEntry(JText::_('JSN_UNIFORM_SUBMENU_CONFIGURATION'), 'index.php?option=com_uniform&view=configuration', $vName == 'configuration');
			JSubMenuHelper::addEntry(JText::_('JSN_UNIFORM_SUBMENU_ABOUT'), 'index.php?option=com_uniform&view=about', $vName == 'about');
		}
	}
	...
	public static function buttonMenu()
	{
		...
		if (JSNVersion::isJoomlaCompatible('2.5'))
		{
			...
			// Add a custom menu button
			$bar->addButtonPath(JPATH_COMPONENT_ADMINISTRATOR . DS . 'helpers');
			$bar->appendButton('JSNMenuButton');
		}
		else
		{
			$bar->appendButton('Custom', self::menuToolbar(), 'jsnmenu');
		}
	}
	 ...
	 * @return type
	 */
	public static function menuAddNew()
	{
		...
		// Generate HTML code for sub-menu
		$html = JSNHtmlGenerate::menuToolbar($options, JText::_('JTOOLBAR_NEW'), 'icon-new icon-white', 'btn btn-small btn-success');
	...
	public static function buttonAddNewForm()
	{
		...
		if (JSNVersion::isJoomlaCompatible('2.5'))
		{
			...
			// Add a custom menu button
			$bar->addButtonPath(JPATH_COMPONENT_ADMINISTRATOR . DS . 'helpers');
			$bar->appendButton('JSNAddNewFormButton');
		}
		else
		{
			$bar->appendButton('Custom', self::menuAddNew(), 'jsnmenu');
		}
	}
