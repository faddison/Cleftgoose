...
class JSNFormGenerateHelper
{
	...
	public static function fieldWebsite($data, $dataSumbission)
	{
		...
		$html = "<div class=\"control-group\"><label for=\"" . htmlentities(JText::_($data->field_title), ENT_QUOTES, "UTF-8") . "\" class=\"control-label\">" . JText::_($data->field_title) . "{$required}{$instruction}</label><div class=\"controls\"><input class=\"website {$requiredWebsite} {$sizeInput}\" id=\"{$data->field_id}\" name=\"{$data->field_id}\" type=\"text\" value=\"{$defaultValue}\" placeholder=\"" . htmlentities($placeholder, ENT_QUOTES, "UTF-8") . "\" /></div></div>";
		return $html;
	}
	...
	public static function fieldCurrency($data, $dataSumbission)
	{
		...
		$html = "<div class=\"control-group\"><label for=\"" . htmlentities(JText::_($data->field_title), ENT_QUOTES, "UTF-8") . "\" class=\"control-label\">" . JText::_($data->field_title) . "{$required}{$instruction}</label><div class=\"controls {$requiredBlank} currency-control clearfix\"><div class=\"clearfix\">{$inputContent}</div></div></div>";
		return $html;
	}
	...
	public static function fieldPhone($data, $dataSumbission)
	{
		...
		$html = "<div class=\"control-group\"><label for=\"" . htmlentities(JText::_($data->field_title), ENT_QUOTES, "UTF-8") . "\" class=\"control-label\">" . JText::_($data->field_title) . "{$required}{$instruction}</label><div class=\"controls {$requiredBlank}\">{$inputContent}</div></div>";
		return $html;
	}
	...
	public static function fieldName($data, $dataSumbission)
	{
		...
		$requiredBlank = !empty($settings->options->required)?'group-blank-required':'';
		$instruction = !empty($settings->options->instruction)?" <i original-title=\"" . htmlentities(JText::_($settings->options->instruction), ENT_QUOTES, "UTF-8") . "\" class=\"icon-question-sign\"></i>":'';
		$html = "<div class=\"control-group\"><label for=\"" . htmlentities(JText::_($data->field_title), ENT_QUOTES, "UTF-8") . "\" class=\"control-label\">" . JText::_($data->field_title) . "{$required}{$instruction}</label><div id=\"{$data->field_id}\" class=\"controls {$requiredBlank}\">";
		$valueFirstName = '';
		$valueLastName = '';
		$valueMiddle = '';
	...
	public static function fieldFileUpload($data, $dataSumbission)
	{
		...
		$required = !empty($settings->options->required)?'<span class="required">*</span>':'';
		$instruction = !empty($settings->options->instruction)?" <i original-title=\"" . htmlentities(JText::_($settings->options->instruction), ENT_QUOTES, "UTF-8") . "\" class=\"icon-question-sign\"></i>":'';
		$html = "<div class=\"control-group\"><label for=\"" . htmlentities(JText::_($data->field_title), ENT_QUOTES, "UTF-8") . "\" class=\"control-label\">" . JText::_($data->field_title) . "{$required}{$instruction}</label><div class=\"controls {$requiredBlank}\"><input id=\"{$data->field_id}\" class=\"input-file\" name=\"{$data->field_id}\" type=\"file\" /></div></div>";
		return $html;
	}
	...
	public static function fieldDropdown($data, $dataSumbission)
	{
		...
		$html = "<div class=\"control-group\"><label for=\"" . htmlentities(JText::_($data->field_title), ENT_QUOTES, "UTF-8") . "\" class=\"control-label\">" . JText::_($data->field_title) . "{$required}{$instruction}</label><div class=\"controls {$requiredBlank}\"><select id=\"{$data->field_id}\" class=\"dropdown {$sizeInput} {$randomDropdown}\" name=\"{$data->field_id}\">";
	...
	public static function fieldList($data, $dataSumbission)
	{
		...
		$html = "<div class=\"control-group\"><label for=\"" . htmlentities(JText::_($data->field_title), ENT_QUOTES, "UTF-8") . "\" class=\"control-label\">" . JText::_($data->field_title) . "{$required}{$instruction}</label><div class=\"controls {$requiredBlank}\"><select {$multiple} id=\"{$data->field_id}\" class=\"list {$sizeInput} {$randomList}\" name=\"{$data->field_id}[]\">";
	...
	public static function fieldCountry($data, $dataSumbission)
	{
		...
		$defaultValue = !empty($dataSumbission[$data->field_id])?$dataSumbission[$data->field_id]:"";
		$sizeInput = !empty($settings->options->size)?$settings->options->size:'';
		$html = "<div class=\"control-group\"><label for=\"" . htmlentities(JText::_($data->field_title), ENT_QUOTES, "UTF-8") . "\" class=\"control-label\">" . JText::_($data->field_title) . "{$required}{$instruction}</label><div class=\"controls {$requiredBlank}\"><select id=\"{$data->field_id}\" class=\"{$sizeInput}\" name=\"{$data->field_id}\">";
		if (isset($settings->options->items) && is_array($settings->options->items))
		{
			foreach ($settings->options->items as $option)
			{
	...
	public static function fieldChoices($data, $dataSumbission)
	{
		...
		$html = "<div class=\"control-group\"><label for=\"" . htmlentities(JText::_($data->field_title), ENT_QUOTES, "UTF-8") . "\" class=\"control-label\">" . JText::_($data->field_title) . "{$required}{$instruction}</label><div class=\"controls {$requiredChoices}\"><div id=\"{$data->field_id}\" class=\"choices jsn-columns-container {$settings->options->layout} {$randomChoices}\">";
	...
	public static function fieldCheckboxes($data, $dataSumbission)
	{
		...
		$html = "<div class=\"control-group\"><label for=\"" . htmlentities(JText::_($data->field_title), ENT_QUOTES, "UTF-8") . "\" class=\"control-label\">" . JText::_($data->field_title) . "{$required}{$instruction}</label><div class=\"controls\"><div id=\"{$data->field_id}\" class=\"checkboxes jsn-columns-container {$settings->options->layout} {$randomCheckbox} {$requiredCheckbox}\">";
		$defaultValue = isset($dataSumbission[$data->field_id])?$dataSumbission[$data->field_id]:"";
		if (isset($settings->options->items) && is_array($settings->options->items))
		{
			foreach ($settings->options->items as $i => $option)
	...
	public static function fieldStaticContent($data, $dataSumbission)
	{
		...
		$html = "<div class=\"control-group\"><div class=\"controls\">{$value}</div></div>";
		return $html;
	}
}