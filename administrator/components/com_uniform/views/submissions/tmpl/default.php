...
<div class="jsn-bootstrap">
	...
	<div class="pull-right jsn-fieldset-select">
		...
		if ($formId)
		{
			...
			<i class="<?php echo JSNVersion::isJoomlaCompatible('2.5')?'icon-list-alt':'icon-list'; ?>"></i>
		    </button>
	    </div>
		...
		<table class="table table-bordered table-striped jsn-table-centered">
		    ...
		    <tr>
			<th width="10">
			    <input type="checkbox" name="toggle" value="" onclick="<?php echo JSNVersion::isJoomlaCompatible('2.5')?'checkAll(' . count($this->_items) . ');':'Joomla.checkAll(this);'; ?>" />
			</th>
			    <?php
			    for ($i = 0; $i < count($fieldIdentifier); $i++)
			    {
