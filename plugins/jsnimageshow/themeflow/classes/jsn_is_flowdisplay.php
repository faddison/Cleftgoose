...
class JSNISFlowDisplay extends JObject
{
	...
	public function standardLayout($args)
	{
		...
		$css			= '#'.$wrapID.' {width:'.$args->width.';height:'.$height.'px;overflow : hidden;margin: 0 auto;position: relative;}';
		...
		if (!empty($imgHeight) AND !empty($imgWidth)) {
			...
			if ($transform == 'transform') {
				...
				$marginTB 	= (int)$themeData->image_border_thickness;
			}
		...
		if ($themeData->image_effect == 'yes') {
			...
			$heightWrapper = (int) $themeData->image_height + ($thickness * 2) - 17;
			$css 		.= '#'.$wrapID.' .wrapper {
				height: ' . $heightWrapper . 'px;
			}';
		}
