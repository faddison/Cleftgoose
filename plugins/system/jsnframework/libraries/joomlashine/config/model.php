...
class JSNConfigModel extends JModelForm
{
	...
	public function getForm($data = array(), $loadData = true, $configFile = '')
	{
		...
		$update = $xml->section[0]->addChild('group');
		$update->addAttribute('name', 'update');
		$update->addAttribute('label', 'JSN_EXTFW_CONFIG_UPDATE');
		$update->addAttribute('icon', 'download');

		$update = $update->addChild('fieldset');
		$update->addAttribute('name', 'update');

		$period = $update->addChild('field');
		$period->addAttribute('name', 'live_update_check_interval');
		$period->addAttribute('type', 'jsntext');
		$period->addAttribute('input-type', 'number');
		$period->addAttribute('default', 6);
		$period->addAttribute('filter', 'int');
		$period->addAttribute('exttext', JText::_('JSN_EXTFW_CONFIG_LIVE_UPDATE_CHECK_INTERVAL_UNIT'));
		$period->addAttribute('class', 'input-small');
		$period->addAttribute('label', 'JSN_EXTFW_CONFIG_LIVE_UPDATE_CHECK_INTERVAL_LABEL');
		$period->addAttribute('description', 'JSN_EXTFW_CONFIG_LIVE_UPDATE_CHECK_INTERVAL_DESC');

		$enable = $update->addChild('field');
		$enable->addAttribute('name', 'live_update_notification');
		$enable->addAttribute('type', 'jsnradio');
		$enable->addAttribute('default', 0);
		$enable->addAttribute('filter', 'int');
		$enable->addAttribute('label', 'JSN_EXTFW_CONFIG_LIVE_UPDATE_NOTIFICATION_LABEL');
		$enable->addAttribute('description', 'JSN_EXTFW_CONFIG_LIVE_UPDATE_NOTIFICATION_DESC');

		$option = $enable->addChild('option', 'JYES');
		$option->addAttribute('value', 1);

		$option = $enable->addChild('option', 'JNO');
		$option->addAttribute('value', 0);

		$action = $update->addChild('action');
