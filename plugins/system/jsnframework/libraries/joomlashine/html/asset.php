...
abstract class JSNHtmlAsset
{
	...
	public static function addScript ($url, $type = "text/javascript", $defer = false, $async = false)
	{
		...
		if (is_array($url))
		{
			foreach ($url as $_url)
			{
				self::addScript($url, $type, $defer, $async);
			}
	 ...
	 * @return  void
	 */
	public static function addInlineScript ($content, $type = 'text/css')
	{
		$document = JFactory::getDocument();
	...
	public static function buildHeader ()
	{
		...
		$html = "
			<script type=\"text/javascript\" src=\"" . JSN_URL_ASSETS . "/3rd-party/requirejs/require.js\" type=\"text/javascript\"></script>
			<script type=\"text/javascript\">
