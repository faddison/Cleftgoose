...
({
	...
	addEvents: function ()
	{
		...
		$(document).addEvents({
			'click': function (e) {
				if(e.target != self.spotlightScrollbar.handle){					
					self.setSpotlightState((self.spotlightKeyword !== undefined && self.spotlightKeyword != '') ? 'inactive' : 'closed');
					dropdownButtons.removeClass('active');
				}								
			},
	...
	addTemplateMenus: function (menu)
	{
		...
		buttons[JoomlaShine.language.JCLOSE] = function (ui) {
			...
			if (currentUrl.indexOf('option=com_poweradmin&view=rawmode') != -1 || currentUrl.indexOf('option=com_templates') != -1) {
	...
	refresh: function ()
	{
		this.toolbar.getParent().setStyle('width', '100%');
